...
- cancellation request status: `pending|accepted|rejected|auto_processed`
...
type ServerModule interface { // cpanel & directadmin adapters implement
    ...
    PackageInUse(ctx, s ServerConfig, name string) (bool, error)              // read-only panel-side guard consulted before DeletePackage: does ANY account on the panel still use the package? Catches accounts the DB sibling-count can't see (created outside this app, or the same physical host registered as a second servers row). cpanel `listaccts` searchtype=package + anchored exact-name regex; DA CMD_API_SHOW_USERS then CMD_API_SHOW_USER_CONFIG per user until a `package=` match (no filter-by-package listing exists in DA's legacy API)
...
- services (client): `GET /services`, `GET /services/:id`, `POST /services/:id/change-password`, `GET /services/:id/sso`, `POST /services/:id/cancel` {mode: immediate|end_of_term}, `POST /services/:id/upgrade` {product_id, cycle, specs?: [{key,qty,unlimited}]} — prorated product/cycle change; `specs` is required knowledge for custom-spec (configurable) targets (unchosen knobs fall back to their default qty, validated/priced server-side exactly like checkout; same product+cycle with different specs = a resize). An upgrade (positive prorated diff) creates a diff invoice (`invoice_items.related_type=service_upgrade`) and stores the pending change (incl. resolved specs) in `services.pending_upgrade` until that invoice is paid — then billing's ProcessPaid → ApplyUpgrade rebinds product/cycle/recurring_amount, rewrites `panel_meta.chosen_specs`, and enqueues `provision:change_package` (which for configurable products rebuilds the dynamic panel package via EnsurePackage and deletes the old dynamic package once no sibling service references it). A downgrade/zero diff applies immediately and credits the excess. `cancel`'s two modes now diverge into a `service_cancellation_requests` row rather than mutating the service directly: `immediate` still cancels open renewal invoices + enqueues termination synchronously (no approval gate), but the request itself starts `pending` too — `ProvisionTerminate` resolves it to `auto_processed` only once the worker actually completes the termination (never at submission time, so a second immediate submission is blocked by the same guard below for as long as the job sits in the queue); `end_of_term` also creates a `pending` request — `panel_meta.cancel_at_period_end` (honoured by AutoTerminate/renewal generation) is set only once an admin accepts it. A service may have at most one `pending` request at a time (service-layer guard + partial unique index), covering both modes. Admin `Accept`/`Reject` only operate on `end_of_term` requests (an immediate request's `pending` row is system-resolved, never an admin decision — `Accept`/`Reject` reject it with `CONFLICT`). Admin: `GET /admin/services/cancellation-requests` {status?, page, per_page} (joined with service domain + client name), `POST /admin/services/cancellation-requests/:id/accept`, `POST /admin/services/cancellation-requests/:id/reject` — both `[perm: services]`, reject leaves the service completely untouched.
...
- admin (prefix /admin, RequireRole admin/staff + permission): clients CRUD+search, orders list/detail/accept/cancel/fraud, invoices CRUD+manual payment+refund+cancel, `POST /admin/invoices/generate-selected` {service_ids[], domain_ids[]} → "Invoice Selected Items" (mirrors WHMCS): force-generates a renewal invoice right now for the given services/domains, no `billing.renewal_lead_days` window check — the only gate is the same eligibility + open-invoice dedupe `GenerateRenewalInvoices` uses (via the same row-locked helpers, so this can never race the cron or itself into a duplicate invoice for the same service/domain); response `{created:[{type,id,invoice_id,invoice_number}], skipped:[{type,id,reason}]}` (both always arrays, never null), services list/detail + `POST /admin/services` — "add existing service" (WHMCS-style): records a pre-existing hosting service directly on a client with NO order, NO payment, NO provisioning job and NO panel call; the row is created `active` (body {client_id, product_id, server_id?, domain?, username?, password? (encrypted at rest), billing_cycle, recurring_amount, next_due_date — required for recurring cycles so renewal invoicing works, registration_date? — defaults to today, notes?}; a server whose module mismatches the product's module is VALIDATION; the client is NOT notified) + `POST /admin/services/:id/{create|suspend|unsuspend|terminate|change-package|change-password}` (sync call to module or enqueue) + `POST /admin/services/:id/upgrade` {product_id, cycle, specs?} — runs the SAME prorated-diff + invoice flow as the client's own `POST /services/:id/upgrade` (an admin executing a billed plan change on a client's behalf, e.g. over a support call), unlike `change-package` which swaps the product immediately with no invoice/proration, domains list/detail/sync/renew/update + `POST /admin/domains` — "add existing domain": records an already-registered domain directly on a client with NO order, NO payment and NO registrar call; attributed to the configured registrar and created `active` (body {client_id, name, next_due_date (required), registration_date?, expiry_date?, recurring_amount, billing_cycle? — default annually, auto_renew? — default true, nameservers? 2-4}; `POST /admin/domains/:id/sync` then pulls the live registrar state) — `PATCH /admin/domains/:id` additionally accepts the billing fields {registration_date, expiry_date, next_due_date, recurring_amount, billing_cycle} (nil = untouched, empty date string = clear), tickets (list/assign/reply/internal note/status), products & groups & pricing & options CRUD, coupons CRUD, servers & server-groups CRUD + test-connection (`POST /admin/servers/test-connection` pre-save probe from form body {module,hostname,port,username,password?,api_token?,use_ssl,id?} + `POST /admin/servers/:id/test-connection` saved-server probe; both → `{ok,message,version?,hostname?,nameservers?}`) + `GET /admin/server-groups/:id/packages` (previews packages on a representative server in the group via `ServerModule.ListPackages`, for the product form's package-name picker; → `{ok,message?,packages?[]}`, OK=false — not an HTTP error — on a panel probe failure so the UI can fall back to manual entry), registrars config + test, gateways config, ticket departments CRUD, email templates CRUD + preview, settings get/put (grouped keys), staff users CRUD + permissions, reports: `GET /admin/dashboard` (KPIs), `GET /admin/reports/revenue?from&to&group_by`, `/admin/reports/orders`, `/admin/reports/services`, audit logs list, email log list, integration logs list.
